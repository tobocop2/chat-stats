...
Normally, the `CHAT_INTERFACE_URL` would not be available in version control because it includes a key avariable, but
...
You will need this if you actually want to see meaningful output from hte API
...
If deployed locally, visit the docs at: bz
...
only create a single redis proces per message.
...
The default dockerredis configuration is used with automatic backups forked in the background every minute
The volume is persisted between the container and the local filesystem

The test suite and the main application have different namespaces configured to avoid key collisoins
...
An RDBMS like postgres was briefly consisdered due to my familiarity.
Fine tuning for updats would make read operations slower. by default postgres is optimized for reads and I reserached
...
would be extremely slow. Since batches are not an option here, I felt that Postgres was out of the picture

In addition to postgres I also considered MySQL since it offers RAM tables, which are supposedly faster than redis
...
* Update tests to properly clear out only test namespace
 - For some reason the command to clear out keys for a namespace isn't working
  so I'm flushing the entire systme when running tests
...
very frequently making commits like

```
"git commit -am "."
```

And subsequently pushing in case I lost power or my laptop died for some reason

I prefer to commit often and push often, making non sensical commits and then rebasing them all into something
that is self explanatory with maybe a few commits each at a working state of code
